...
export default function Header() {
  ...
  const LoginMyPageButton = () => {
    ...
    if (cookies.accessToken)
      return (
        <div className="mypage-button" onClick={onMyPageButtonClickHandler}>
          마이페이지
        </div>
      );
    //          render: 로그인 버튼 컴포넌트 렌더링 (로그인 상태가 아닐 때)         //
    return (
      <div className="login-button" onClick={onLoginButtonClickHandler}>
        로그인
      </div>
    );
  };
  ...
  const UploadButton = () => {
    ...
    if (title && contents)
      return (
        <div className="upload-button" onClick={onUploadButtonClickHandler}>
          업로드
        </div>
      );
  ...
  const UserPageButtons = () => {
    ...
    if (isMyPage)
      return (
        <div className="logout-button" onClick={onLogoutButtonClickHandler}>
          로그아웃
        </div>
      );
