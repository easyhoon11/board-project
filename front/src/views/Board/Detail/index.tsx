...
export default function BoardDetail() {
  ...
  //					component: 게시물 상세보기 상단 페이지 컴포넌트					//
      ...
      <div id="board-detail-top">
        <div className="board-detail-header">
          ...
          <div className="board-detail-sub-box">
            <div className="board-detail-write-info-box">
              ...
              <div className="board-detail-writer-date">
                {board?.writeDatetime}
              </div>
            </div>
            {isWriter && (
        ...
        <div className="board-detail-top-main">
          ...
          {board?.imageUrls.map((imageUrl) => (
            <img className="board-drtail-main-image" src={imageUrl} />
          ))}
        </div>
      </div>
    );
  };
  //					component: 게시물 상세보기 하단 페이지 컴포넌트					//
  const BoardDetailBottom = () => {
    ...
    const onCommentChangeHandler = (
      ...
      setComment(comment);
      // description: textarea 내용 바뀔때 마다 높이 변경 //
      if (!textareaRef.current) return;
      textareaRef.current.style.height = "auto";
      textareaRef.current.style.height = `${textareaRef.current.scrollHeight}px`;
    };

    //            effect: 게시물 번호가 path variable이 바뀔 때 마다 좋아요 및 댓글 리스트 불러오기          //
    useEffect(() => {
      setFavoriteList(favoriteListMock);
      setBoardList(commentListMock);
      setCommentCount(commentListMock.length);
    }, [boardNumber]);
    //						render: 게시물 상세보기 상단 페이지 렌더링						//
    return (
      <div id="board-detail-bottom">
        <div className="board-detail-bottom-button-box">
          ...
          <div className="board-detail-bottom-button-group">
            ...
            <div className="board-detail-bottom-button-text">{`댓글 ${commentCount}`}</div>
            <div
              className="icon-button"
              onClick={onShowCommentsButtonClickHandler}
            >
              {showComment ? (
                <div className="up-light-icon"></div>
              ) : (
          ...
          <div className="board-detail-bottom-favorite-box">
            <div className="board-detail-bottom-favorite-container">
              <div className="board-detail-bottom-favorite-title">
                {"좋아요 "}
                <span className="empgasis">{`${favoriteList.length}`}</span>
              </div>
              <div className="board-detail-bottom-favorite-contents">
                {favoriteList.map((favoriteItem) => (
                  <div className="board-detail-bottom-favorite-item">
                    <div
                      className="board-detail-bottom-favorite-profile-image"
                      style={{
                        backgroundImage: `url(${ favoriteItem.profileImage ? favoriteItem.profileImage : DefaultProfileImage})`,
                      }}
                    ></div>
                    <div className="board-detail-bottom-favorite-nickname">
                      {favoriteItem.nickname}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="board-detail-bottom-comments-box">
            <div className="board-detail-bottom-comments-container">
              <div className="board-detail-bottom-comments-list-container">
                <div className="board-detail-bottom-comments-list-title">
                  {"댓글 "}
                  <span className="empgasis">{commentCount}</span>
                </div>
            ...
            {user !== null && (
              <div className="board-detail-bottom-comments-input-box">
                <div className="board-detail-bottom-comments-input-container">
                  <textarea
                    ref={textareaRef}
                    className="board-detail-bottom-comments-input"
