...
export default function BoardDetail() {
      ...
      <div id="board-detail-top">
          ...
          <div className="board-detail-sub-box">
            <div className="board-detail-write-info-box">
              ...
              <div className="board-detail-write-date">
                {getWriteDatetimeFormat(board?.writeDatetime)}
              </div>
            </div>
            {isWriter && (
        ...
        <div className="board-detail-top-main">
          ...
          {board?.boardImageList.map((boardImage) => (
            <img className="board-detail-main-image" src={boardImage} />
          ))}
        </div>
      </div>
    );
  ...
  const BoardDetailBottom = () => {
    ...
    const getFavoriteListResponse = (
      responseBody: GetFavoriteListResponseDto | ResponseDto
    ) => {
      const { code } = responseBody;
      if (code === "NB") alert("존재하지 않는 게시물입니다.");
      if (code === "DBE") alert("데이터베이스 오류입니다.");
      ...
      const isFavorite =
        favoriteList.findIndex((item) => item.email === user?.email) !== -1;
    ...
    const onCommentButtonClickHandler = () => {
      ...
      postCommentRequest(requestBody, boardNumber, accessToken).then(
        postCommentResponse
      );
    };
            ...
            <div className="board-detail-bottom-comments-container">
              <div className="board-detail-bottom-comments-list-container">
                <div className="board-detail-bottom-comments-list-title">
                  {"댓글 "}
                  <span className="emphasis">{commentsCount}</span>
                </div>
              ...
              <div className="board-detail-bottom-comments-input-box">
                <div className="board-detail-bottom-comments-input-container">
                  ...
                  <div className="board-detail-bottom-comments-button-box">
                    {comment.length === 0 ? (
                      <div className="board-detail-bottom-comments-button-disable">
                        {"댓글달기"}
                      </div>
                    ) : (
                      <div
                        className="board-detail-bottom-comments-button"
                        onClick={onCommentButtonClickHandler}
                      >
                        {"댓글달기"}
                      </div>
                    )}
