...
export default function Main() {
  ...
  const MainBottom = () => {
    ...
    const getLatestBoardListResponse = (
      responseBody: GetLatestBoardListResponseDto | ResponseDto
    ) => {
      const { code } = responseBody;
      if (code === "DBE") alert("데이터 베이스 오류 입니다.");
      if (code !== "SU") return;
    ...
    useEffect(() => {
      getPopularWordListRequest().then(getPopularListResponse);
              ...
              <div className="main-bottom-popular-word-card">
                <div className="main-bottom-popular-card-box">
                  ...
                  <div className="main-bottom-popular-card-contents">
                    {popularWordList.map((popularWord) => (
                      <div
                        className="word-bedge"
                        onClick={() => onWordBadgeClickHandler(popularWord)}
                      >
                        {popularWord}
                      </div>
                    ))}
