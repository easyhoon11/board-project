...
export default function Search() {
  ...
  const onWordBedgeClickHandler = (word: string) => {
  ...
  useEffect(() => {
    ...
    getRelationWordListRequest(word).then(getRelationListResponse);
  }, [word, effectFlag]);
      ...
      <div className="search-container">
        <div className="search-title-box">
          <div className="search-title">
            <span className="search-title-emphasis">{word}</span>
            {"에 대한 검색결과 입니다."}
          </div>
        ...
        <div className="search-contents-box">
          {count === 0 ? (
            <div className="search-contents-result-nothing">
              {"검색 결과가 없습니다."}
            </div>
          ) : (
              ...
              <div className="search-relation-card-box">
                ...
                {relationWordList.length === 0 ? (
                  ...
                  <div className="search-relation-card-contents">
                    {relationWordList.map((relationWord) => (
                      <div
                        className="word-bedge"
                        onClick={() => onWordBedgeClickHandler(relationWord)}
                      >
                        {relationWord}
                      </div>
                    ))}
